...
struct {
	...
	float maxFightTime = 0 //Lucien2714: default to no max fight time, set it to > 0 to enable max fight time, send both players to waiting lobby after max time reached.
...
void function endSpectate(entity player) {
	...
	try {
		player.Die(null, null, {
			damageSourceId = eDamageSourceId.damagedef_despawn
		})
	} catch (error) {}
...
void function AddPlayerToWaitingList(soloPlayerStruct playerStruct) {
	if (IsValid(playerStruct.player) && isInputAllowed(playerStruct.player)) {
		...
		#if DEVELOPER
		sqprint(format("[AddPlayerToWaitingList] player: %s, input allowed: %s", playerStruct.player.p.name, isInputAllowed(playerStruct.player) ? "true" : "false"))
		#endif
		file.soloPlayersWaiting[playerStruct.player.p.handle] <- playerStruct
	} else {
		#if DEVELOPER
		sqprint(format("[AddPlayerToWaitingList] Failed to add player to waiting list: input allowed:%s, player valid:%s", isInputAllowed(playerStruct.player) ? "true" : "false", IsValid(playerStruct.player) ? "true" : "false"))
		#endif
	}
				...
				if (!removed && !quit) {
					...
					foreach(eachPlayer in players) {
						...
						if (Distance2D(eachPlayer.GetOrigin(), Center) > settings.playerMaxFightDistance) {
							...
							eachPlayer.TakeDamage(1, null, null, {
								scriptType = DF_BYPASS_SHIELD | DF_DOOMED_HEALTH_LOSS,
								damageSourceId = eDamageSourceId.deathField
							})
						}
					}
				}
...
//Made by @CafeFPS
//This function returns the input of the player
//Returns 0 for mnk, 1 for controller
...
bool function GroupIsLockable(soloGroupStruct newGroup) { //(mk): This can return "could not lock" message when the enemy has not moved yet for the match. Intended behavior.
...
void function Gamemode1v1_OnPlayerKilled(entity victim, entity attacker,
	var damageInfo) {
	if (IsValid(attacker))
	...
	if (Gamemode1v1_IsPlayerWaiting(victim)) //Shouldn't happen, but just in case
	{
		...
		if (!IsValid(waitingRoomLocation)) { //(mk): this should never be hit, Maki had it checked.
